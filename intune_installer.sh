...
"fedora")
  ...
  # Validate supported release
  if [[ "$MAJOR" != "42" && "$MAJOR" != "43" ]]; then
    die "$DISTRO $RELEASE is not supported. Supported major versions: 42, 43"
  fi
  ...
  # RHEL 10+ uses a newer Microsoft GPG signing key
  if [[ "$MAJOR" -ge 43 ]]; then
    MAJOR=10 # RHEL 10 and Fedora 43+ use the same key
    MS_GPG_KEY="https://packages.microsoft.com/rhel/$MAJOR/$CHANNEL/repodata/repomd.xml.key"
  elif [[ "$MAJOR" -ge 42 ]]; then
    MAJOR=9 # RHEL 9 and Fedora 42 use the same key
    MS_GPG_KEY="https://packages.microsoft.com/keys/microsoft.asc"
  else
    die "Fedora $RELEASE is not supported. Supported versions: 42, 43"
  fi

  # For portal and dependencies
  sudo dnf config-manager addrepo --overwrite --from-repofile=https://packages.microsoft.com/config/rhel/$MAJOR/prod.repo
